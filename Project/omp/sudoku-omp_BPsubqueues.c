	...
	#pragma omp task shared(finish)
		if(!finish) {
			...
			if(q_node != NULL) {
				...
				int n_levels = (to_solve->n_plays - q_node->next_ptr > MAX_LEVELS) ? MAX_LEVELS : to_solve->n_plays - q_node->next_ptr;
				...
				while(1) {
					...
					if(!finish) {
						...
						if(safe_by_square(q_node->plays, get_square_by_ptr(q_node->next_ptr), plays[ptr])) {
